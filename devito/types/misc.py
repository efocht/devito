...
class FIndexed(Indexed, Pickable):
    ...
    def __new__(cls, indexed, suffix):
        plabel = Symbol(name='%s%s' % (indexed.name, suffix.upper()), dtype=indexed.dtype)
        ...
        slabel = Symbol(name='%s%s' % (indexed.name, suffix.lower()), dtype=indexed.dtype)
